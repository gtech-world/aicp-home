    ...
    <div className="flex flex-col justify-between p-5 mt-5 mr-5 bg-white rounded-lg mo:mr-0 box-layout mo:w-full">
      <div>
        ...
        <div className="mt-5">
          {infos.map((v: { label: string; text: string }, i: number) => {
            return <PartInfo key={`PartInfo${i}`} label={v.label} text={v.text} />;
          })}
        </div>
      </div>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex mt-5 mo:flex-col">
          {btn.map((v: { type: string; text: string; onClick?: () => void }, i: number) => {
  ...
  const inProgress = useMemo<
    {
      ...
      icon: any;
      infos: any[];
      btn: { onClick: () => void; text: string; type?: string }[];
    }[]
  >(
    () => [
      {
      ...
      },
      {
        ...
        infos: [
          {
            label: '服务内容介绍',
            text: '针对企业应对双碳问题的现状梳理与科学碳目标设定。服务可涉及通用的气候治理架构、应对双碳问题的战略框架、减碳目标、碳中和等，也可为企业参与常见的目标、承诺或倡议活动提供辅导与支持，例如SBTi、RE100等。',
          },
    ...
    () => [
      {
        title: (
          <span>
            [AI01]<span className="text-green-2">[202303444101]</span>公司碳核查
          </span>
        ),
      ...
      },
      {
        title: (
          <span>
            [AI01]<span className="text-green-2">[202303443042]</span>公司碳核查
          </span>
        ),
        icon: <SvgCTag className="w-[3.45rem]" />,
        infos: [
    ...
    <Fragment>
      <div className="flex flex-col items-start">
        <div className="flex flex-wrap items-center cursor-pointer" onClick={() => setInProgressOpen(!inProgressOpen)}>
          ...
          <FaSortDown
            className={classNames('ml-2 mt-[-0.45rem] text-2xl', inProgressOpen && 'rotate-180 mb-[-1.2rem]')}
          />
        </div>
        {inProgressOpen && (
          <div className="flex flex-wrap mr-[-20px] mo:flex-col  mo:mr-0">
            {inProgress.map((v, i) => {
              return <Card key={`inProgress${i}`} data={v} />;
            })}
          </div>
        )}
      </div>
      <div className="flex flex-col items-start mt-5">
        <div className="flex flex-wrap items-center cursor-pointer" onClick={() => setCompletedOpen(!completedOpen)}>
          ...
          <FaSortDown
            className={classNames('ml-2 mt-[-0.45rem] text-2xl', completedOpen && 'rotate-180 mb-[-1.2rem]')}
          />
        </div>
        {completedOpen && (
          <div className="flex w-full flex-wrap mr-[-20px] mo:flex-col  mo:mr-0">
            {completed.map((v, i) => {
              return <Card key={`inProgress${i}`} data={v} />;
            })}
          </div>
