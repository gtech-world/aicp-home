...
import {
  ...
  CNeutralization as SvgCNeutralization,
  CTag as SvgCTag,
  Energy as SvgEnergy,
    ...
    () => [
      ...
      },
      {
        ...
        infos: [
          {
            label: '服务内容介绍',
            text: '针对汽车行业各类企业可能需要的双碳/低碳发展主题提供的咨询服务。包括：企业碳减排方案、ISCC国际可持续与碳认证、CDP/GRI碳披露、能源管理体系、ESG信息披露与核查等。',
          },
    ...
    () => [
      ...
      },
      {
        ...
        infos: [
          {
            label: '服务内容',
            text: '基于减碳活动产生的碳信用(Carbon Credit)资产的开发管理。服务可包含：前期咨询评估、开发管理、市场资源。',
          },
      ...
      },
      {
        ...
        infos: [
          {
            label: '服务内容',
            text: '帮助用户在自愿碳市场购买和交易碳信用类资产，并完成碳抵销(Carbon Offsetting)活动。',
          },
    ...
    <Fragment>
      ...
      <div className="flex flex-col items-start mt-5">
        <div className="flex flex-wrap items-center cursor-pointer" onClick={() => setImplementOpen(!implementOpen)}>
          ...
          <FaSortDown
            className={classNames('ml-2 mt-[-0.45rem] text-2xl', implementOpen && 'rotate-180 mb-[-1.2rem]')}
          />
        </div>
        {implementOpen && (
          <div className="flex flex-wrap mr-[-20px] mo:flex-col  mo:mr-0">
            {implement.map((v, i) => {
              return <Card key={`implement${i}`} data={v} />;
            })}
          </div>
        )}
      </div>
      <div className="flex flex-col items-start mt-5">
        ...
        {marketOpen && (
          <div className="flex flex-wrap mr-[-20px] mo:flex-col  mo:mr-0">
            {market.map((v, i) => {
              return <Card key={`market${i}`} data={v} />;
            })}
          </div>
        )}
      </div>
