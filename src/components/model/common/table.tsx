...
export function ContentTable(p: { head: string[]; className?: string; infos: ReactNode[][] }) {
  const { head, className, infos } = p;
  return (
    <div
...
export function Table(p: { infos: ReactNode[][]; title: string; head: string[]; className?: string }) {
  ...
  return (
    <div className={className}>
      ...
      {show && <ContentTable head={head} infos={infos} className="mt-3" />}
    </div>
  );
}
