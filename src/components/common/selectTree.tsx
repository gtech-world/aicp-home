...
const getNodeData = (node: any, nestingLevel: any, onSelect: (node: any) => void, onSwitch: (node: any) => void) => ({
  data: {
  ...
  return (
    ...
    >
      <button
        ...
        className={classNames('text-xl text-gray-9 mr-[4px]', {
          invisible: isLeaf,
        })}
      >
...
export function SelectTree(p: { node: any; onChange: (node: any) => void; classname?: string }) {
    ...
    <div className="" ref={ref}>
      <div
        className={classNames(
          'w-full justify-between h-10 cursor-pointer rounded-lg flex items-center px-3',
          classname,
        )}
      ...
      {open && (
        <AutoSizer>
          {({ width }: any) => (
            <div
              className="bg-white rounded-lg mt-2.5 p-5 absolute"
              ref={mtreeRef}
